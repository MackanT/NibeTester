...
class NibeSerial:
        ...
        while len(self.read_buffer) > 0:
            ...
            if msg:
                ...
                if len(self.read_buffer) < 50:
                    ...
                    logger.warning(f"Discarding byte from large buffer: 0x{self.read_buffer[0]:02X}")
                    self.read_buffer.pop(0)
    ...
    def _handle_message(self, msg: NibeMessage):
        ...
        if msg.command == MessageType.ANNOUNCEMENT or msg.command == MessageType.DATA_RESPONSE:
            self.send_ack()
