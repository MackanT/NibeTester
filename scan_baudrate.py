...
def main():
    ...
    for baudrate in BAUD_RATES:
        ...
        if 'error' in result:
            ...
            print(f"Bytes: {result['total_bytes']}, Valid starts: {result['valid_starts']}")
    ...
    for i, result in enumerate(results_sorted, 1):
        ...
        if result['valid_starts'] > 0:
            print(f"   ⭐ RECOMMENDED - Found {result['valid_starts']} valid start bytes!")
    
    if results_sorted and results_sorted[0]['valid_starts'] > 0:
        print()
        print("=" * 70)
        print(f"✓ Best match: {results_sorted[0]['baudrate']} baud")
        print(f"  Update test_nibe.py line 30: baudrate={results_sorted[0]['baudrate']}")
        print("=" * 70)
    else:
        print()
        print("=" * 70)
